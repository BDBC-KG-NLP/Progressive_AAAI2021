...
class ToyNet(nn.Module):
    def __init__(self, args):
        ...
        # GRU for P(Trc|S,Y')
        self.GRU_mean_rc = torch.nn.Linear(len(constant.BIO_TO_ID)+len(constant.LABEL_TO_ID)+self.rnn_hidden*2, self.rnn_hidden*2)
        self.GRU_std_rc = torch.nn.Linear(len(constant.BIO_TO_ID)+len(constant.LABEL_TO_ID)+self.rnn_hidden*2, self.rnn_hidden*2)

        # GRU for P(Tner|S,Y')
        self.GRU_mean_ner = torch.nn.Linear(len(constant.BIO_TO_ID)+len(constant.LABEL_TO_ID)+self.rnn_hidden*2, self.rnn_hidden*2)
        self.GRU_std_ner = torch.nn.Linear(len(constant.BIO_TO_ID)+len(constant.LABEL_TO_ID)+self.rnn_hidden*2, self.rnn_hidden*2)
    ...
    def get_statistics_batch(self, embeds, task):
        if task == 'rc':
            ...
            mean = self.GRU_mean_rc(torch.cat([Y,H], dim=1)) 
            std = self.GRU_std_rc(torch.cat([Y,H], dim=1)) 
            ...
            Y = embeds[0].reshape(-1, len(constant.BIO_TO_ID)+len(constant.LABEL_TO_ID))
            H = embeds[1].reshape(-1, self.rnn_hidden*2)
            mean = self.GRU_mean_ner(torch.cat([Y,H], dim=1)) 
            std = self.GRU_std_ner(torch.cat([Y,H], dim=1)) 
            # reshape to bsz, sqlen, hidden_size
            mean = mean.reshape(embeds[1].size(0), embeds[1].size(1), -1)
            std = std.reshape(embeds[1].size(0), embeds[1].size(1), -1)
        elif task == 'rc_r': # t|y'
