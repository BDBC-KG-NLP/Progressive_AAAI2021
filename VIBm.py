...
class ToyNet(nn.Module):
    def __init__(self, args):
        ...
        self.r_std = nn.Parameter(torch.randn(self.max_sent_len, self.K, self.K))
        self.r_diag = nn.Parameter(torch.randn(self.max_sent_len, self.K))
        # orthogonal initialization r_std
        for i in range(self.max_sent_len):
            nn.init.orthogonal_(self.r_std[i], gain=1)
    ...
    def forward(self, inputs, num_sample=1):
        ...
        # ner prediction
        ner_logit = self.ner_cla(encoding)
        ...
        # diag elements > 0
        std_diag = self.r_diag[:seqlen]*self.r_diag[:seqlen]+SMALL
        # orthogonal loss
        E_matrix = torch.eye(std_r.size(1)).cuda().unsqueeze(0).repeat(std_r.size(0),1,1)
        orthogonal_loss = self.loss_fn(std_r.bmm(std_r.transpose(-1,-2)), E_matrix)
        mean, cov = mean, cov
        kl_div = self.kl_div((mean, cov), (mean_r, std_r, std_diag), real_len, mask_kl)

        return kl_div.mean(), orthogonal_loss, ner_logit.view(-1, len(constant.BIO_TO_ID)), rc_logit.view(-1, len(constant.LABEL_TO_ID))
