...
class ToyNet(nn.Module):
    def __init__(self, args):
        ...
        # GRU
        self.GRUg = torch.nn.GRUCell(len(constant.BIO_TO_ID), self.rnn_hidden*2)
        self.GRUr = torch.nn.GRUCell(len(constant.LABEL_TO_ID), self.rnn_hidden*2)
    ...
    def forward(self, inputs, num_sample=1):
        ...
        # get task specific features Hg and Hr
        Hg = self.GRUg(prob_Cg.reshape(-1, len(constant.BIO_TO_ID)), H.reshape(-1, self.args.rnn_hidden*2))
        Hr = self.GRUr(prob_Cr.reshape(-1, len(constant.LABEL_TO_ID)), H.reshape(-1, self.args.rnn_hidden*2))
        Hg = Hg.view(H.size(0), H.size(1), self.args.rnn_hidden*2)
        Hr = Hr.view(H.size(0), H.size(1), self.args.rnn_hidden*2)
        H = H+Hr+Hg
        
        # compress H for transfering information to Hr and Hg
        # for Hr
        ...
        # repeat Hg, Hr and mask
        Hg = Hg.unsqueeze(1).repeat(1, num_sample, 1, 1)
        Hr = Hr.unsqueeze(1).repeat(1, num_sample, 1, 1)
