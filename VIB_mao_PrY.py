...
class ToyNet(nn.Module):
    ...
    def get_statistics_batch(self, embeds, task):
        if task == 'rc':
            ...
            mean = self.r_mean_rc(embeds) 
            std = self.r_std_rc(embeds)
        elif task == 'ner_r': # t|y' 
            mean = self.r_mean_ner(embeds) 
            std = self.r_std_ner(embeds)        
        cov = std * std + SMALL
        return mean, cov
    ...
    def forward(self, inputs, num_sample=1):
        ...
        # P(Trc|S,Y') 
        Y = torch.tensor(torch.cat([prob_Cr, prob_Cg], dim=2)).cuda()
