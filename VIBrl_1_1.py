...
class ToyNet(nn.Module):
    ...
    def kl_div(self, param1, param2, real_len, mask_kl):
        ...
        logdet_up_tri = torch.sum(torch.sum(torch.log(torch.sum(up_tri*eye_diag, dim=1)), dim=1).reshape(bsz, seqlen)*mask_kl, dim=1)
        term1 = 2.0*logdet_up_tri - torch.sum(torch.sum(torch.log(cov1), dim=2)*mask_kl, dim=1) - var_len

        term2 = torch.sum(torch.sum(torch.sum(cov2_inv*cov1.reshape(-1,tag_dim).unsqueeze(1)*eye_diag, dim=1), dim=1).reshape(bsz, seqlen)*mask_kl, dim=1)

        mean_diff = (mean2 - mean1).reshape(-1, tag_dim)
        term3 = torch.sum(mean_diff.unsqueeze(1).bmm(cov2_inv).bmm(mean_diff.unsqueeze(-1)).reshape(bsz, seqlen)*mask_kl, dim=1)
