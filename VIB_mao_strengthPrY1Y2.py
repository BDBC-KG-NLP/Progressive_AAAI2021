...
class ToyNet(nn.Module):
    def __init__(self, args):
        ...
        # GRU for P(Trc|S,Y')
        self.GRU_mean_rc = torch.nn.GRUCell(len(constant.LABEL_TO_ID), self.rnn_hidden*2)
        self.GRU_std_rc = torch.nn.GRUCell(len(constant.LABEL_TO_ID), self.rnn_hidden*2)

        # GRU for P(Tner|S,Y')
        self.GRU_mean_ner = torch.nn.GRUCell(len(constant.BIO_TO_ID), self.rnn_hidden*2)
        self.GRU_std_ner = torch.nn.GRUCell(len(constant.BIO_TO_ID), self.rnn_hidden*2)
        
        # define r
        self.r_mean_rc = nn.Linear(len(constant.LABEL_TO_ID), self.rnn_hidden*2)
        self.r_mean_rc2 = nn.Linear(self.rnn_hidden*2, self.rnn_hidden*2)
        self.r_std_rc = nn.Linear(len(constant.LABEL_TO_ID), self.rnn_hidden*2)
        self.r_std_rc2 = nn.Linear(self.rnn_hidden*2, self.rnn_hidden*2)
        self.r_mean_ner = nn.Linear(len(constant.BIO_TO_ID), self.rnn_hidden*2)
        self.r_mean_ner2 = nn.Linear(self.rnn_hidden*2, self.rnn_hidden*2)
        self.r_std_ner = nn.Linear(len(constant.BIO_TO_ID), self.rnn_hidden*2)
    ...
    def get_statistics_batch(self, embeds, task):
        if task == 'rc':
            Y = embeds[0].reshape(-1, len(constant.LABEL_TO_ID))
            ...
            Y = embeds[0].reshape(-1, len(constant.BIO_TO_ID))
            H = embeds[1].reshape(-1, self.rnn_hidden*2)
    ...
    def forward(self, inputs, num_sample=1):
        ...
        Y1 = prob_Cr
        Y2 = prob_Cg
        mean_rc, cov_rc = self.get_statistics_batch((Y1, H), task='rc')
        encoding_rc = self.get_sample_from_param_batch(mean_rc, cov_rc, num_sample)
        # P(Tner|S,Y')
        mean_ner, cov_ner = self.get_statistics_batch((Y2, H), task='ner')
        encoding_ner = self.get_sample_from_param_batch(mean_ner, cov_ner, num_sample)
        # P(Trc|Y')
        mean_r_rc, cov_r_rc = self.get_statistics_batch(torch.tensor(Y1).cuda(), task='rc_r')
        # P(Tner|Y')
        mean_r_ner, cov_r_ner = self.get_statistics_batch(torch.tensor(Y2).cuda(), task='ner_r')
