...
class ToyNet(nn.Module):
    ...
    def kl_div(self, param1, param2, real_len, mask_kl):
        ...
        eye_diag = torch.eye(up_tri.size(-1)).unsqueeze(0).repeat(bsz,1,1).cuda()
        mask_kl = mask_kl.view(bsz,-1)
        logdet_up_tri = torch.sum(torch.log(torch.sum(up_tri*eye_diag, dim=1))*mask_kl, dim=1)
        term1 = 2.0*logdet_up_tri - torch.sum(torch.log(cov1)*mask_kl, dim=1) - var_len
        
        cov2_inv = cov2.inverse()
        term2 = torch.sum(torch.sum(cov2_inv*cov1.unsqueeze(1)*eye_diag, dim=1)*mask_kl, dim=1)

        mean_diff = mean1 - mean2
        mean_diff = mean_diff.view(bsz, -1)*mask_kl
        term3 = mean_diff.unsqueeze(1).bmm(cov2_inv).bmm(mean_diff.unsqueeze(-1))
    ...
    def forward(self, inputs, num_sample=1):
        ...
        std_r = self.r_std[:seqlen*self.K]
        std_r = std_r.mm(std_r.transpose(-1,-2))+SMALL
        cov_r = std_r.unsqueeze(0).expand(bsz,-1,-1)
        mean, cov = mean, cov
